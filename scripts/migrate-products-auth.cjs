...
async function main() {
  ...
  // Sign in as admin if service role key is not provided
  if (!isServiceRole) {
    console.log("Signing in with admin credentials to obtain auth session...");
    const { data: authData, error: authErr } = await supabase.auth.signInWithPassword({
      email: 'admin@minora.com',
      password: 'Rudrik@900',
    });

    if (authErr) {
      console.error("[AUTH FAILURE] Admin sign in failed:", authErr.message);
    } else {
      console.log(`[AUTH SUCCESS] Signed in as Admin (${authData.user.email})`);
    }
  }

  // Query count before migration
  ...
  // Map fields exactly to public.products schema
  ...
  if (upsertErr) {
    console.error("\n[UPSERT FAILED] Supabase Error:", upsertErr.message);
