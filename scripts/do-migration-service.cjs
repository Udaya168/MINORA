...
if (!serviceRoleKey) {
  console.error("Please provide SUPABASE_SERVICE_ROLE_KEY environment variable to run server-side migration.");
  console.error("Usage: SUPABASE_SERVICE_ROLE_KEY=your_key node scripts/do-migration-service.cjs");
  process.exit(1);
}
...
async function runMigration() {
  ...
  const { count: prevCount } = await supabase.from('products').select('*', { count: 'exact', head: true });
  console.log(`Previous Supabase count: ${prevCount}`);
  ...
  // 2. Upsert using Service Role Key
  ...
  console.log(`Number migrated: ${upsertData ? upsertData.length : 0}`);

  // 3. Verify New Count
  const { count: newCount } = await supabase.from('products').select('*', { count: 'exact', head: true });
  console.log(`New Supabase count: ${newCount}`);
  ...
  console.log("\nSample Migrated Products Verification (First 10):");
